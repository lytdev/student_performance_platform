...
def ajaxcomperformance(request):
    ...
    for i in comperformancescores:
        arecord = [unicode(i.comperformance.term), unicode(i.student.realname), unicode(i.student.user.username), unicode(i.student.theclass.classid),
                   unicode(i.score)]
        ...
        try:
            ...
            for i in classes:
                series.append({
                    'name': i,
                    'data': [round(
                        ComperformanceScore.objects.filter(student__theclass__classid=i).filter(comperformance__term=j).aggregate(avgprice=Avg('score'))[
                            'avgprice'], 2) for j in xAxis]
                })
...
def ajaxupdatecomperformance(request):
    if request.is_ajax():
        ...
        for i in xrange(Behavior.objects.count()):
            ...
            if dict(request.GET)['data[]'][i]:
                comperformancebehaviorscore, created = ComperformanceBehaviorScore.objects.get_or_create(comperformance=comperformance, student=student,
                ...
                if created:
                    ...
                    comperformancescore.score = comperformancescore.score + float(
                        dict(request.GET)['data[]'][i]) * 0.3 - comperformancebehaviorscore.score * 0.3
                    comperformancebehaviorscore.score = float(dict(request.GET)['data[]'][i])
        ...
        for i in xrange(Development.objects.count()):
            if dict(request.GET)['data[]'][num + i]:
                comperformancedevelopmentscore, created = ComperformanceDevelopmentScore.objects.get_or_create(comperformance=comperformance, student=student,
